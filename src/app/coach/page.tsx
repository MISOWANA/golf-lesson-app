  ...
  return (
    <div className="px-4 py-6">
      <div className="mb-6 flex items-center justify-between">
        <div>
          <h1 className="text-xl font-bold">안녕하세요, {userName || '코치'}님 👋</h1>
      ...
      {showInvite && (
        <Card className="mb-6 border-2 border-green-200 bg-green-50">
          <p className="mb-1 text-xs font-medium text-green-700">회원 초대 코드</p>
          <div className="flex items-center justify-between">
            <span className="text-2xl font-bold tracking-widest text-green-800">{inviteCode}</span>
      ...
      {loading ? (
        ...
        <div className="py-16 text-center">
          ...
          <p className="mt-3 font-medium text-gray-500">연결된 회원이 없습니다</p>
          <p className="mt-1 text-sm text-gray-400">회원 초대 버튼으로 초대 코드를 생성해보세요</p>
        </div>
      ) : (
        <div className="space-y-3">
          {members.map(m => (
            <Link key={m.memberId} href={`/coach/members/${m.memberId}`}>
              <Card className="active:scale-98 flex items-center gap-4 transition-transform">
                ...
                <div className="text-right">
                  <p className="text-sm font-bold text-green-700">{m.totalLessons}회</p>
                  <p className="text-xs text-gray-400">잔여 {m.remainingLessons}회</p>
                </div>
              </Card>
            </Link>
          ))}
